...
def beet_default(ctx: Context):
    mc = ctx.inject(Mecha)
    mc.providers.append(FileTypeCompilationUnitProvider([TestFile]))
...
class TestRootParser:
    ...
    def __call__(self, stream: TokenStream):
        if "test_file" not in stream.data:
            test_file = isinstance(self.database.current, TestFile)
    ...
    def nested_test(self, node: AstCommand):
        if isinstance(resource_location := node.arguments[0], AstResourceLocation):
            ...
            target = self.generate(
                full_name, default=TestFile(original=self.database.current.original)
            )
...
def convert_tests(ctx: Context):
    ...
    for name, test in ctx.data[TestFile].items():
        ctx.data[name] = Function(test.text)
    ctx.data[TestFile].clear()
